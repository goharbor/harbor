...
For example:
```
    CREATE USER harbor_admin WITH PASSWORD 'myPasswordfromharbor.cfg';
    GRANT ALL ON DATABASE postgres to harbor_admin;
    CREATE USER clair_admin WITH PASSWORD 'myPasswordfromharbor.cfg';
    GRANT ALL ON DATABASE postgres to clair_admin
    \q
```
