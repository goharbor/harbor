...
To be specific, edit or create the file `/etc/docker/daemon.json`. Add the following contents:
```json
{
    "insecure-registries" : ["reg.yourdomain.com:5000"]
}
```
And, if this config with hostname `reg.yourdomain.com` doesn't take effort, try to replace `reg.yourdomain.com:5000` with `0.0.0.0/0`. Just like this:
```json
{
    "insecure-registries" : ["0.0.0.0/0"]
}
```
Then, restart your docker daemon with
```sh
