...
Edit the file ```harbor.yml```, update the hostname and the protocol, and update the attributes ```certificate``` and ```private_key```:
...
# https related config
 https:
   ...
   certificate: /data/harbor/harbor/ssl/harbor.cluster.local.crt
   private_key: /data/harbor/harbor/ssl/yourdomain.com.key
```
