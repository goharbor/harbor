...
func ReversibleEncrypt(str, key string) (string, error) {
	...
	encrypted := ENCRYPT_HEADER_V1 + base64.StdEncoding.EncodeToString(cipherText)
...
func ReversibleDecrypt(str, key string) (string, error) {
	if strings.HasPrefix(str, ENCRYPT_HEADER_V1) {
		str = str[len(ENCRYPT_HEADER_V1):]
