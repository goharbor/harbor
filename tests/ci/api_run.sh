...
E2E_IMAGE_UI="goharbor/harbor-e2e-engine:latest-ui"
LOCAL_REGISTRY="registry.goharbor.io"
LOCAL_REGISTRY_NAMESPACE="harbor"
...
if [ "$1" = 'DB' ]; then
    ...
    docker run -i --privileged -v $DIR/../../:/drone -v $DIR/../:/ca -w /drone $E2E_IMAGE_UI robot --exclude proxy_cache -v DOCKER_USER:"${DOCKER_USER}" -v DOCKER_PWD:${DOCKER_PWD} -v ip:$2  -v ip1: -v http_get_ca:false -v LOCAL_REGISTRY:${LOCAL_REGISTRY} -v LOCAL_REGISTRY_NAMESPACE:${LOCAL_REGISTRY_NAMESPACE} -v HARBOR_PASSWORD:${HARBOR_ADMIN_PASSWD} -v HARBOR_ADMIN:${HARBOR_ADMIN} /drone/tests/robot-cases/Group1-Nightly/Setup.robot /drone/tests/robot-cases/Group1-Nightly/Routing.robot
