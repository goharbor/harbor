...
function publishImageGhcr {
    ...
    docker images --format "table {{.Repository}}\t{{.Tag}}" | grep goharbor | grep -v "\-base" | grep -v "harbor-db" | grep -v "valkey" | grep -E "^goharbor/harbor-|^goharbor/registry-photon" | grep -v "^goharbor/prepare" | grep "$version" || echo "NO MATCHES FOUND for version $version"

    # Count images that will be pushed
    image_count=$(docker images --format "table {{.Repository}}\t{{.Tag}}" \
      | grep goharbor | grep -v "\-base" | grep -v "harbor-db" | grep -v "valkey" | grep -E "^goharbor/harbor-|^goharbor/registry-photon" | grep -v "^goharbor/prepare" \
      | grep "$version" | wc -l)
