...
def push_image_to_project(project_name, registry, username, password, image,
        tag, expected_login_error_message = None, expected_error_message = None,
        profix_for_image = None, new_image=None, src_registry="docker"):
    print("Start to push image {}/{}/{}:{}".format(registry, project_name, image, tag) )
    _docker_api = DockerAPI()
    _docker_api.docker_login(src_registry, DOCKER_USER, DOCKER_PWD)
