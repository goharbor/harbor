...
class TestProjects(unittest.TestCase):
    ...
    def testSignImage(self):
        ...
        src_tag = "latest"
        #5. Create a new repository(RA) and tag(TA) in project(PA) by user(UA);
        TestProjects.repo_name, tag = create_repository(project_add_g_lbl_name, harbor_server, user_add_g_lbl_name, user_001_password, image, src_tag)
        set_sign_env(harbor_server, project_add_g_lbl_name, image, tag)

        self.repo.signature_should_exist(TestProjects.repo_name, tag, **TestProjects.USER_add_g_lbl_CLIENT)
