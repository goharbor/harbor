...
Prepare Accessory
    ...
    # --- Step 1: Extract SBOM digest ---
    Retry Double Keywords When Error  Retry Button Click  ${artifact_sbom_accessory_action_btn}  Wait Until Element Is Visible And Enabled  ${copy_digest_btn}
    Retry Button Click  ${copy_digest_btn}
    Wait Until Element Is Visible And Enabled  ${artifact_digest}
    ${sbom_digest}=  Get Text  ${artifact_digest}
    Run Keyword And Ignore Error  Retry Double Keywords When Error  Retry Button Click  ${copy_btn}  Retry Wait Element Not Visible  ${copy_btn}
    
    # --- Step 2: Extract Signature digest ---
    Retry Double Keywords When Error  Retry Button Click  ${artifact_cosign_accessory_action_btn}  Wait Until Element Is Visible And Enabled  ${copy_digest_btn}
    Retry Button Click  ${copy_digest_btn}
    Wait Until Element Is Visible And Enabled  ${artifact_digest}
    ${signature_digest}=  Get Text  ${artifact_digest}
    Run Keyword And Ignore Error  Retry Double Keywords When Error  Retry Button Click  ${copy_btn}  Retry Wait Element Not Visible  ${copy_btn}
    ...
    # --- Step 4: Extract Signature of SBOM digest (Level 3) ---
    Retry Double Keywords When Error  Retry Button Click  ${artifact_list_sbom_accessory_btn}  Wait Until Element Is Visible And Enabled  ${artifact_sbom_cosign_accessory_action_btn}
    Retry Button Click  ${artifact_sbom_cosign_accessory_action_btn}
    Retry Button Click  ${copy_digest_btn}
    Wait Until Element Is Visible And Enabled  ${artifact_digest}
    ${signature_of_sbom_digest}=  Get Text  ${artifact_digest}
    Run Keyword And Ignore Error  Retry Double Keywords When Error  Retry Click Element  ${copy_btn}  Retry Wait Element Not Visible  ${copy_btn}
    
    # --- Step 5: Extract Signature of Signature digest (Level 3) ---
    Retry Double Keywords When Error  Retry Button Click  ${artifact_list_cosign_accessory_btn}  Wait Until Element Is Visible And Enabled  ${artifact_cosign_cosign_accessory_action_btn}
    Retry Button Click  ${artifact_cosign_cosign_accessory_action_btn}
    Retry Button Click  ${copy_digest_btn}
    Wait Until Element Is Visible And Enabled  ${artifact_digest}
    ${signature_of_signature_digest}=  Get Text  ${artifact_digest}
    Run Keyword And Ignore Error  Retry Double Keywords When Error  Retry Click Element  ${copy_btn}  Retry Wait Element Not Visible  ${copy_btn}
