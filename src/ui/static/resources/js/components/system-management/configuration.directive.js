...
(function() {
  ...
  function ConfigurationController($scope, ConfigurationService, defaultPassword, $filter, trFilter) {
    ...
    var confKeyDefinitions = {
      ...
      'email_server': { type: 'email', attr: 'server' },
      'email_server_port': { type: 'email', attr: 'serverPort' },
      ...
      'max_job_workers': { type: 'system', attr: 'maxJobWorkers' }
    };
    ...
    function getConfigurationSuccess(response) {
      for(var key in response) {
        ...
        if(mappedDef) {
          $scope[mappedDef['type']][mappedDef['attr']] = { 'target': mappedDef['type'] + '.' + mappedDef['attr'], 'data': valueMapping(response[key]['value']) };
          $scope.$watch(mappedDef['type'] + '.' + mappedDef['attr'], onChangedCallback, true);
          $scope[mappedDef['type']][mappedDef['attr']]['origin'] = { 'target': mappedDef['type'] + '.' + mappedDef['attr'], 'data': valueMapping(response[key]['value']) };
          vm.editable[mappedDef['type'] + '.' + mappedDef['attr']] = response[key]['editable'];
        }
      }
    ...
    function valueMapping(value) {
      switch(value) {
      case 'on':
        return vm.toggleOptions[0];
      case 'off':
        return vm.toggleOptions[1];
      case 'true':
      ...
      for(var key in confKeyDefinitions) {
        ...
        for(var itemKey in vm.changedItems) {
          ...
          if (compositeKey === itemKey) {
            (typeof item === 'object' && item) ? vm.updatedItems[key] = item.value : vm.updatedItems[key] = item || '';
          }
        }
      }
    }
