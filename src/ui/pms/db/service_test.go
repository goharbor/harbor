...
func TestGetRoles(t *testing.T) {
	...
	// non exist user
	assert.Equal(t, []int{},
		pms.GetRoles("non_exist_user", int64(1)))

	// project ID
	assert.Equal(t, []int{common.RoleProjectAdmin},
		pms.GetRoles("admin", int64(1)))

	// project name
	assert.Equal(t, []int{common.RoleProjectAdmin},
		pms.GetRoles("admin", "library"))

	// non exist project
	assert.Equal(t, []int{},
		pms.GetRoles("admin", "non_exist_project"))

	// invalid type
	assert.Equal(t, []int{}, pms.GetRoles("admin", 1))
}
