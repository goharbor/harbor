...
func (l *LabelAPI) List() {
	...
	if len(scope) == 0 {
		l.HandleBadRequest("scope is required")
		return
	}
	...
	if scope == common.LabelScopeProject {
		...
		exist, err := l.ProjectMgr.Exists(projectID)
		if err != nil {
			l.HandleInternalServerError(fmt.Sprintf("failed to check the existence of project %d: %v", projectID, err))
			return
		}
		if !exist {
			l.HandleNotFound(fmt.Sprintf("project %d not found", projectID))
			return
		}
