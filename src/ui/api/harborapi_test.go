...
func init() {
	...
	beego.Router("/api/admin/jobs", &AdminJobAPI{}, "put:Put;post:Post")
	beego.Router("/api/admin/jobs/?:name", &AdminJobAPI{}, "get:Get")
	beego.Router("/api/admin/jobs/:id([0-9]+)/log", &AdminJobAPI{}, "get:GetLog")
...
func (a testapi) AddAdminJob(authInfor usrInfo, adminReq apilib.AdminJobReq) (int, error) {
	_sling := sling.New().Post(a.basePath)
...
func (a testapi) AdminJobGet(authInfo usrInfo) (int, []apilib.AdminJob, error) {
	_sling := sling.New().Get(a.basePath)
	path := "/api/admin/jobs/gc"
	_sling = _sling.Path(path)
	httpStatusCode, body, err := request(_sling, jsonAcceptHeader, authInfo)
