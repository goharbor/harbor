...
// CheckAndTriggerReplication search trigger first, if found, then trigger the replication
func CheckAndTriggerReplication(image, operation string) {
	project, _ := utils.ParseRepository(image)
	watchItem := trigger.DefaultWatchList.Get(project)
	if watchItem == nil {
		log.Debugf("no replication trigger found for resource %s, skip")
		return
	}

	if !(operation == "push" && watchItem.OnPush ||
		operation == "delete" && watchItem.OnDeletion) {
		log.Debugf("no replication trigger found for resource: %s, operation: %s, skip", image, operation)
		return
	}
	...
	log.Debugf("replication for resource: %s, operation: %s triggered", image, operation)
}
