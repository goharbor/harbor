  ...
  deleteTag(tag: TagView) {
    if (tag) {
      ...
      if (tag.signed) {
        ...
        confirmOnly = true;
        content = 'notary -s https://' + this.registryUrl + ':4443 --tlscacert ~/.docker/tls/' + this.registryUrl + ':4443/ca.crt -d ~/.docker/trust remove -p ' + this.registryUrl + '/' + this.repoName + ' ' + tag.tag;
      } else {
