...
func TestParseClusterURL(t *testing.T) {
	url := "redis+cluster://anonymous:password@host1:26379,host2:26379?idle_timeout_seconds=30&max_retries=10&min_retry_backoff=1&max_retry_backoff=10&dial_timeout=30&read_timeout=5&write_timeout=5&pool_fifo=true&pool_size=1000&min_idle_conns=100&max_conn_age=10&pool_timeout=10"
	o, err := ParseClusterURL(url)
	assert.NoError(t, err)
	assert.Equal(t, "anonymous", o.Username)
	assert.Equal(t, "password", o.Password)
	assert.Equal(t, []string{"host1:26379", "host2:26379"}, o.Addrs)
	assert.Equal(t, 30*time.Second, o.IdleTimeout)
	assert.Equal(t, 10, o.MaxRetries)
	assert.Equal(t, 10*time.Second, o.MaxRetryBackoff)
	assert.Equal(t, 1*time.Second, o.MinRetryBackoff)
