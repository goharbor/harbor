...
func (suite *CacheTestSuite) TestNew() {
	...
	{
		c, err := New("mock://?codec=foo")
		suite.NotNil(c)
		suite.Nil(err)
	}
}

func (suite *CacheTestSuite) TestDefaultCodec() {
	codec := newCodec("default")

	{
		var i int
		b, err := codec.Encode(10)
		suite.Nil(err)

		err = codec.Decode(b, &i)
		suite.Nil(err)
		suite.Equal(10, i)
	}
}
...
func (suite *CacheTestSuite) TestInitialize() {
	...
	{
		Register("ping", func(opts Options) (Cache, error) {
			c := &cachetesting.Cache{}
			c.On("Ping").Return(fmt.Errorf("failed"))
