...
func TestValidatePublicHTTPURL(t *testing.T) {
	tests := []struct {
		...
		// Unlike the well-known /96 prefix above, the local-use /48 prefix (RFC 8215)
		// is blocked outright regardless of the embedded IPv4 address.
		{name: "NAT64 local-use prefix blocked outright", rawURL: "http://[64:ff9b:1:101:1:100::]/hook", wantErr: true},
	}
...
func TestValidatePublicHTTPURLAllowPrivateIP(t *testing.T) {
	tests := []struct {
		name   string
		rawURL string
	}{
		{name: "loopback", rawURL: "http://127.0.0.1/hook"},
		{name: "localhost", rawURL: "http://localhost/hook"},
		{name: "private IPv4", rawURL: "http://10.0.0.1/hook"},
		{name: "metadata IPv4", rawURL: "http://169.254.169.254/latest/meta-data"},
		{name: "IPv6 loopback", rawURL: "http://[::1]/hook"},
		{name: "IPv6 unique local", rawURL: "http://[fc00::1]/hook"},
		{name: "cluster local", rawURL: "http://service.namespace.svc.cluster.local/hook"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Sanity check: the same target is rejected without the opt-in flag,
			// so a bug that made allowPrivate a no-op wouldn't slip through as a
			// vacuously-passing test.
			_, err := ValidatePublicHTTPURL(context.Background(), tt.rawURL, false)
			require.Error(t, err)

			allowCtx := context.WithValue(context.Background(), AllowPrivateIPKey, true)
			_, err = ValidatePublicHTTPURL(allowCtx, tt.rawURL, false)
			require.NoError(t, err)
		})
	}
}

func TestValidatePublicHTTPURLAllowPrivateIPResolvedHost(t *testing.T) {
	...
	// Without the opt-in flag, a hostname resolving to a private IP is still blocked.
	_, err := ValidatePublicHTTPURL(context.Background(), "https://internal.example.com/hook", true)
	require.Error(t, err)

	allowCtx := context.WithValue(context.Background(), AllowPrivateIPKey, true)
	_, err = ValidatePublicHTTPURL(allowCtx, "https://internal.example.com/hook", true)
	require.NoError(t, err)
}

func TestPublicDialAddressAllowsPrivateIPWhenFlagSet(t *testing.T) {
	allowCtx := context.WithValue(context.Background(), AllowPrivateIPKey, true)

	// Without the opt-in flag, dialing a private IP is rejected before any dial is attempted.
	_, err := PublicDialAddress(context.Background(), "127.0.0.1", "8080")
	require.Error(t, err)

	address, err := PublicDialAddress(allowCtx, "127.0.0.1", "8080")
	require.NoError(t, err)
