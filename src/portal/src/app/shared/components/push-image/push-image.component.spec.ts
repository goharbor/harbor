...
describe('PushImageButtonComponent (inline template)', () => {
  ...
  it('should open the drop-down panel',  async () => {
    ...
    expect(copyInputs[0].value.trim()).toEqual(`docker tag SOURCE_IMAGE[:TAG] https://testing.harbor.com/testing/REPOSITORY[:TAG]`);
    expect(copyInputs[1].value.trim()).toEqual(`docker push https://testing.harbor.com/testing/REPOSITORY[:TAG]`);
  });
});
...
@Component({
  template: `
    <hbr-push-image-button [projectName]="'testing'" [registryUrl]="'https://testing.harbor.com'">
