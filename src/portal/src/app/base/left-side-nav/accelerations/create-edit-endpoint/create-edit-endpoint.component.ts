...
import { clone, compareValue, CURRENT_BASE_HREF, isEmptyObject } from "../../../../shared/units/utils";
import { HttpClient } from "@angular/common/http";
import { catchError } from "rxjs/operators";
import { EndpointService, HELM_HUB } from "../../../../shared/services/endpoint.service";
import { Endpoint, PingEndpoint } from "src/app/shared/services";
...
@Component({
  selector: "hbr-create-edit-accelerations",
...
export class CreateEditEndpointComponent
  ...
  openCreateEditTarget(editable: boolean, targetId?: number | string) {
    ...
    if (targetId) {
      ...
      this.translateService
        ...
        .subscribe(res => (this.modalTitle = 'Edit Acceleration Endpoint'));
      this.endpointService.getAccelerationEndpoint(targetId).subscribe(
        target => {
          this.target = target;
      ...
      this.translateService
        ...
        .subscribe(res => (this.modalTitle = 'New Acceleration Endpoint'));
  ...
  testConnection() {
    ...
    this.endpointService.pingAccelerationEndpoint(payload).subscribe(
      response => {
  ...
  addEndpoint() {
    ...
    this.onGoing = true;
    this.endpointService.createAccelerationEndpoint(this.target).subscribe(
      response => {
        this.translateService
  ...
  updateEndpoint() {
    ...
    this.onGoing = true;
    this.endpointService.updateAccelerationEndpoint(this.target.id, payload).subscribe(
      response => {
        this.translateService
