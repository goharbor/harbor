...
import { Comparator, Endpoint, RequestQueryParams } from "../../../shared/services";
...
@Component({
    selector: "hbr-accelerations",
    templateUrl: "./accelerations.component.html",
    styleUrls: ["./accelerations.component.scss"],
})
export class AccelerationsComponent implements OnInit, OnDestroy {
    ...
    retrieve(state?: ClrDatagridStateInterface): void {
        this.selectedRow = [];
        this.loading = true;
        let queryParams = new RequestQueryParams();
        queryParams = queryParams.set("page", this.page);
        queryParams = queryParams.set("pageSize", this.pageSize);
        this.oldEndpointService.getAccelerations(this.targetName, queryParams
        ).pipe(finalize(() => {
            this.loading = false;
        }))
            .subscribe(response => {
                ...
                if (this.targets?.length) {
                    this.targets.forEach(item => {
                       this.oldEndpointService.pingAccelerationEndpoint(item).subscribe(() => {
                           item.status = 'healthy';
                       }, () => {
    ...
    delOperate(target: Registry): Observable<any> {
        ...
        return this.oldEndpointService.deleteEndpoint(
            target.id
        ).pipe(map(
                response => {
