...
func newAdapter(registry *model.Registry) (*awsecr, error) {
	region, err := parseRegion(registry.URL)
	if err != nil {
		return nil, err
	}
	authorizer := NewAuth(region, registry.Credential.AccessKey, registry.Credential.AccessSecret)
...
var _ adp.Adapter = awsecr{}

func (awsecr) Info() (info *model.RegistryInfo, err error) {
...
func (d awsecr) HealthCheck() (model.HealthStatus, error) {
	var err error
...
func (d awsecr) PrepareForPush(resources []*model.Resource) error {
...
func (d awsecr) createRepository(repository string) error {
