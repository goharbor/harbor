...
func TestIsAuthenticated(t *testing.T) {
	...
	// authenticated
	ctx = NewSecurityContext(&role.Role{
		Role: model.Role{
			Name: "test",
		},
	})
...
func Test_filterRolePolicies(t *testing.T) {
	type args struct {
		p        *proModels.Project
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := filterRolePolicies(tt.args.p, tt.args.policies); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("filterRolePolicies() = %v, want %v", got, tt.want)
			}
		})
	}
}
