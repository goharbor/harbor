...
func NewMySQL(host, port, usr, pwd, database, collation string, maxIdleConns int, maxOpenConns int) Database {
	return &mysql{
		host:         host,
		port:         port,
		usr:          usr,
...
func (m *mysql) Register(alias ...string) error {
	...
	conn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?collation=%s&parseTime=true&loc=Local", m.usr,
		m.pwd, m.host, m.port, m.database, m.collation)
