...
// SetSystemConfigEditable - only used in UT environment
func SetSystemConfigEditable() {
	systemConfigEditable = true
}
...
func (s *ConfigureStore) UpdateConfigValue(keyName string, value string) error {
	itemMetadata, err := MetaData.GetConfigMetaData(keyName)
	if systemConfigEditable || itemMetadata.Scope == UserScope {
