...
type tokenGenerator func(realm, service string, scopes []*Scope) (token string, expiresIn int, issuedAt *time.Time, err error)
...
func (t *tokenAuthorizer) Authorize(req *http.Request, params map[string]string) error {
	...
	if expired || hasFrom {
		to, expiresIn, _, err := t.tg(params["realm"], params["service"], scopes)
		if err != nil {
			return err
		}
		token = to
...
func (s *standardTokenAuthorizer) generateToken(realm, service string, scopes []*Scope) (string, int, *time.Time, error) {
	tk, err := getToken(s.client, s.credential, realm, service, scopes)
...
func (u *usernameTokenAuthorizer) genRegistryToken(realm, service string, scopes []*Scope) (token string, expiresIn int, issuedAt *time.Time, err error) {
	token, expiresIn, issuedAt, err = token_util.RegistryTokenForUI(u.username, service, scopeStr(scopes))
	return
}

func (u *usernameTokenAuthorizer) genNotaryToken(realm, service string, scopes []*Scope) (token string, expiresIn int, issuedAt *time.Time, err error) {
	token, expiresIn, issuedAt, err = token_util.NotaryTokenForUI(u.username, service, scopeStr(scopes))
	return
}

func scopeStr(scopes []*Scope) []string {
	str := []string{}
	for _, scope := range scopes {
		str = append(str, scope.string())
	}
	return str
}
