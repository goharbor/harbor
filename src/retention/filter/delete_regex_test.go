...
func TestNewDeleteRegex(t *testing.T) {
	tests := []struct {
		Name        string
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			sut, err := NewDeleteRegex(tt.Metadata)
			...
			if tt.ExpectedErr != nil {
				...
				assert.NotNil(t, sut)
				assert.NotNil(t, sut.(*deleteRegex).match)
			}
		})
	}
}

func TestDeleteRegex_Process(t *testing.T) {
	sut := &deleteRegex{match: regexp.MustCompile(`^[abc]+-\d+$`)}

	tests := []struct {
		Tag            string
		ExpectedAction retention.FilterAction
	}{
		{Tag: "abc-123", ExpectedAction: retention.FilterActionDelete},
		{Tag: "a2c-123", ExpectedAction: retention.FilterActionNoDecision},
		{Tag: "a-1", ExpectedAction: retention.FilterActionDelete},
		{Tag: "ab-12", ExpectedAction: retention.FilterActionDelete},
