...
// API is the base API handler
type API struct{}

// Prepare default prepare for operation
func (*API) Prepare(ctx context.Context, operation string, params interface{}) middleware.Responder {
	return nil
}

// SendError returns response for the err
func (*API) SendError(ctx context.Context, err error) middleware.Responder {
	return errs.NewErrResponder(err)
}

// HasPermission returns true when the request has action permission on resource
func (*API) HasPermission(ctx context.Context, action rbac.Action, resource rbac.Resource) bool {
...
func (a *API) HasProjectPermission(ctx context.Context, projectIDOrName interface{}, action rbac.Action, subresource ...rbac.Resource) bool {
	...
	return a.HasPermission(ctx, action, resource)
}
...
func (a *API) RequireProjectAccess(ctx context.Context, projectIDOrName interface{}, action rbac.Action, subresource ...rbac.Resource) error {
	if a.HasProjectPermission(ctx, projectIDOrName, action, subresource...) {
		return nil
	}
...
func (a *API) RequireSysAdmin(ctx context.Context) error {
	secCtx, ok := security.FromContext(ctx)
	if !ok {
		return errors.UnauthorizedError(errors.New("security context not found"))
	}
...
func (a *API) BuildQuery(ctx context.Context, query *string, pageNumber, pageSize *int64) (*q.Query, error) {
	var (
		qs string
		pn int64
		ps int64
...
func (a *API) Links(ctx context.Context, u *url.URL, total, pageNumber, pageSize int64) lib.Links {
	var links lib.Links
	if pageSize == 0 {
		return links
	}
