...
const (
	...
	upstreamRegistryLimitLevel = "UPSTREAM_REGISTRY_LIMIT_LEVEL" // if UPSTREAM_REGISTRY_LIMIT_LEVEL is project, the upstream registry connection is based on project level, by default it is artifact level
)
...
func handleBlob(w http.ResponseWriter, r *http.Request, next http.Handler) error {
	...
	if p.MaxUpstreamConnection() > 0 {
		...
		log.Debugf("handle blob key: %s", key)
...
func upstreamRegistryConnectionKey(art lib.ArtifactInfo) string {
	level := os.Getenv(upstreamRegistryLimitLevel)
	if strings.EqualFold("project", level) {
