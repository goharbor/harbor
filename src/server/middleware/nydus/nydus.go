...
/*
/v2/library/hello-world/manifests/latest-nydus
{
    "schemaVersion": 2,
    "config": {
...
func AcceleratorMiddleware() func(http.Handler) http.Handler {
	return middleware.AfterResponse(func(w http.ResponseWriter, r *http.Request, statusCode int) error {
		...
		log.Info("Start NydusAccelerator Middleware")
		ctx := r.Context()
		...
		for _, descriptor := range manifest.References() {
			log.Info("descriptor.Annotations", descriptor.Annotations)
			log.Info("mediaTypeNydusLayer", mediaTypeNydusLayer)
			annotationMap := descriptor.Annotations
			if _, ok := annotationMap["containerd.io/snapshot/nydus-bootstrap"]; ok {
				if annotationMap["containerd.io/snapshot/nydus-bootstrap"] == "true" {
					isNydus = true
					break
				}
			}
		...
		if isNydus {
			subjectArt, err := artifact.Ctl.GetByReference(ctx, info.Repository, mf.Annotations["io.goharbor.artifact.v1alpha1.acceleration.source.digest"], nil)
			if err != nil {
