...
func TestConvertFilterValue(t *testing.T) {
	// Test label filter with array input
	result := convertFilterValue(model.FilterTypeLabel, []interface{}{"label1", "label2"})
	labelArray, ok := result.([]interface{})
	assert.True(t, ok, "Label filter should return []interface{}")
	assert.Len(t, labelArray, 2)
	assert.Equal(t, "label1", labelArray[0])
	assert.Equal(t, "label2", labelArray[1])

	// Test label filter with string slice input
	result2 := convertFilterValue(model.FilterTypeLabel, []string{"label3", "label4"})
	labelArray2, ok := result2.([]interface{})
	assert.True(t, ok, "Label filter should return []interface{}")
	assert.Len(t, labelArray2, 2)
	assert.Equal(t, "label3", labelArray2[0])
	assert.Equal(t, "label4", labelArray2[1])

	// Test label filter with comma-separated string
	result3 := convertFilterValue(model.FilterTypeLabel, "env=prod,team=backend")
	labelArray3, ok := result3.([]interface{})
	assert.True(t, ok, "Label filter should return []interface{}")
	assert.Len(t, labelArray3, 2)
	assert.Equal(t, "env=prod", labelArray3[0])
	assert.Equal(t, "team=backend", labelArray3[1])

	// Test non-label filter (should return as-is)
	result4 := convertFilterValue(model.FilterTypeTag, "latest")
	assert.Equal(t, "latest", result4)

	// Test creating a filter and validating it passes
	filter := &model.Filter{
		Type:       model.FilterTypeLabel,
		Value:      convertFilterValue(model.FilterTypeLabel, []interface{}{"test=label"}),
		Decoration: "matches",
	}
	err := filter.Validate()
	assert.NoError(t, err, "Label filter with []interface{} should validate successfully")
}