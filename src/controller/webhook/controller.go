...
var (
	...
	// webhookJobVendors represents webhook(http), slack, or telegram.
	webhookJobVendors = q.NewOrList([]any{job.WebhookJobVendorType, job.SlackJobVendorType, job.TelegramJobVendorType})
)
...
func (c *controller) DeletePolicy(ctx context.Context, policyID int64) error {
	...
	// there are three vendor types(webhook, slack & telegram) needs to be deleted.
