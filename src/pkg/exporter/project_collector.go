...
var (
	...
	projectBasicSQL = `SELECT project.project_id, project.name, COALESCE(project_metadata.value, 'false') AS public, COALESCE(quota.hard, '{"storage": -1}') AS quota, COALESCE(quota_usage.used, '{}') AS usage FROM project
	LEFT JOIN project_metadata ON (project.project_id = project_metadata.project_id AND project_metadata.name='public')
	LEFT JOIN quota ON (CAST(project.project_id AS text) = quota.reference_id AND quota.reference='project')
	LEFT JOIN quota_usage ON (CAST(project.project_id AS text) = quota_usage.reference_id AND quota_usage.reference='project')
