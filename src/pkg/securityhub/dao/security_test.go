...
func (suite *SecurityDaoTestSuite) TestRepositoryFilter() {
	type args struct {
		ctx   context.Context
		key   string
		query *q.Query
	...
	tests := []struct {
		...
		{"normal", args{suite.Context(), "repository", q.New(q.KeyWords{"repository": "library/nginx"})}, " and a.repository_name = ?", []interface{}{"library/nginx"}},
	}
	for _, tt := range tests {
		suite.Run(tt.name, func() {
			gotSQLStr, gotParams := repositoryFilter(tt.args.ctx, tt.args.key, tt.args.query)
