...
func (m *mgrTestSuite) TestOnboard() {
	...
	m.dao.On("Create", mock.Anything, testifymock.MatchedBy(
		func(u *models.User) bool {
			return u.Username == "dup-but-not-existing"
		})).Return(0, errors.ConflictError(nil).WithMessage("conflict for other reason"))
	m.dao.On("Create", mock.Anything, testifymock.MatchedBy(
		func(u *models.User) bool {
	...
	{
		newUser := &models.User{
			Username: "dup-but-not-existing",
			Email:    "dup-but-not-existing@mytest.com",
			Realname: "dup-but-not-existing",
		}
		err := m.mgr.Onboard(context.Background(), newUser)
		m.NotNil(err)
