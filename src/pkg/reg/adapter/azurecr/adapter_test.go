...
//
// Run from the src directory with environment variables:
//
//	cd src
...
func TestChunkedUploadToACR(t *testing.T) {
	...
	// unique repository per run to avoid cross-run interference
	repository := fmt.Sprintf("test/chunked-upload-test-%d", time.Now().UnixNano())
	...
	t.Cleanup(func() {
		if err := client.DeleteBlob(repository, digest); err != nil {
			t.Logf("cleanup: failed to delete blob %s from %s: %v", digest, repository, err)
		}
	})
