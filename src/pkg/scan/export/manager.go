...
inner join scanner_registration on scan_report.registration_uuid = scanner_registration.uuid`
	...
	TagIdColumn                  = "tag.id"
	CVEIDColumn                  = "vulnerability_record.cve_id"
...
func (em *exportManager) buildQuery(ctx context.Context, params Params, artifactsWithLabel []int64) (beego_orm.RawSeter, error) {
	...
	if len(filterFragment) > 0 {
		sql = fmt.Sprintf("%s %s %s", VulnScanReportQuery, SQLAnd, filterFragment)
	}
...
func (em *exportManager) getFilters(ctx context.Context, params Params, artifactsWithLabel []int64) (string, error) {
	...
	if params.CVEIds != nil && len(params.CVEIds) > 0 {
		filters = append(filters, fmt.Sprintf("%s in (%s)", CVEIDColumn, strings.Join(params.CVEIds, ",")))
	}

	// fetch any label specific artifact id filters
	/*
		artifactIds, err := em.getArtifactsWithLabel(ctx, params.Labels)
		if err != nil {
			return "", err
		}*/
