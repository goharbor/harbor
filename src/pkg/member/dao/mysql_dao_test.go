...
func (s *MysqlDaoTestSuite) SetupSuite() {
	s.Suite.SetupSuite()
...
func (s *MysqlDaoTestSuite) TearDownSuite() {
}

func (s *MysqlDaoTestSuite) TestAddProjectMember() {
	ctx := s.Context()
	proj, err := s.projectMgr.Get(ctx, "member_test_01")
	s.Nil(err)
	s.NotNil(proj)
...
func (s *MysqlDaoTestSuite) TestUpdateProjectMemberRole() {
	ctx := s.Context()
	proj, err := s.projectMgr.Get(ctx, "member_test_01")
	s.Nil(err)
	s.NotNil(proj)
	...
	pmid, err := s.dao.AddProjectMember(ctx, member)
	s.Nil(err)
	err = s.dao.UpdateProjectMemberRole(ctx, proj.ProjectID, pmid, common.RoleDeveloper)
	s.Nil(err)
...
func (s *MysqlDaoTestSuite) TestGetProjectMembers() {
	ctx := s.Context()
...
func (s *MysqlDaoTestSuite) TestGetTotalOfProjectMembers() {
	ctx := s.Context()
...
func (s *MysqlDaoTestSuite) TestListRoles() {
	ctx := s.Context()
...
func (s *MysqlDaoTestSuite) TestDeleteProjectMember() {
	ctx := s.Context()
...
func (s *MysqlDaoTestSuite) TestDeleteProjectMemberByUserId() {
	ctx := s.Context()
	userID := 22
	var addMember = models.Member{
		ProjectID:  s.projectID,
