...
type processor struct {
	...
	evaluators map[*rule.Evaluator][]reselector.Selector
	// action performer
...
func New(parameters []*alg.Parameter) alg.Processor {
	p := &processor{
		evaluators: make(map[*rule.Evaluator][]reselector.Selector),
...
func (p *processor) Process(artifacts []*reselector.Candidate) ([]*reselector.Result, error) {
	if len(artifacts) == 0 {
		log.Debug("no artifacts to retention")
		return make([]*reselector.Result, 0), nil
	}
	...
	type chanItem struct {
		action    string
		processed []*reselector.Candidate
	}
	...
	for eva, selectors := range p.evaluators {
		...
		go func(evaluator rule.Evaluator, selectors []reselector.Selector) {
			var (
				processed []*reselector.Candidate
				err       error
			)
	...
	results := make([]*reselector.Result, 0)
	// Perform actions
...
type cHash map[string]*reselector.Candidate

func (ch cHash) toList() []*reselector.Candidate {
	l := make([]*reselector.Candidate, 0)
