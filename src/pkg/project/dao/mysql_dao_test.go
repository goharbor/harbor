...
func (suite *MysqlDaoTestSuite) WithUser(f func(int64, string), usernames ...string) {
	var username string
	if len(usernames) > 0 {
	...
	sql := "INSERT INTO harbor_user (username, realname, email, password) VALUES (?, ?, ?, 'Harbor12345')"
	_, err = o.Raw(sql, username, username, email).Exec()
	suite.Nil(err)
	selectVersionSQL := "SELECT user_id FROM harbor_user WHERE username = ?"
	suite.Nil(o.Raw(selectVersionSQL, username).QueryRow(&userID))
...
func (suite *MysqlDaoTestSuite) WithUserGroup(f func(int64, string), groupNames ...string) {
	...
	sql := "INSERT INTO user_group (group_name, ldap_group_dn) VALUES (?, ?)"
	res, err := o.Raw(sql, groupName, groupDN).Exec()
	suite.Nil(err)
	selectSQL := "SELECT id FROM user_group WHERE group_name = ?"
	suite.Nil(o.Raw(selectSQL, groupName).QueryRow(&groupID))
	insertID, err := res.LastInsertId()
	suite.Equal(groupID, insertID)
...
func (suite *MysqlDaoTestSuite) TestGetByName() {
	{
...
func (suite *MysqlDaoTestSuite) TestListByPublic() {
	{
...
func (suite *MysqlDaoTestSuite) TestListByOwner() {
	{
		// default library project
...
func (suite *MysqlDaoTestSuite) TestListByMember() {
	{
		// project admin
...

	{
		suite.WithUser(func(userID int64, username string) {
			...
			suite.WithUserGroup(func(groupID int64, groupName string) {
				...
				res, err := o.Raw(sql, projectID, groupID, common.RoleGuest, "g").Exec()
				suite.Nil(err)
				insertID, err := res.LastInsertId()
				selectPidSQL := "SELECT project_id FROM project_member WHERE id = ?"
				suite.Nil(o.Raw(selectPidSQL, insertID).QueryRow(&pid))
				suite.Equal(projectID, pid)
...
func (suite *MysqlDaoTestSuite) TestListRoles() {
	{
		// only projectAdmin
		suite.WithUser(func(userID int64, username string) {
			project := &models.Project{
