...
PLAYWRIGHT_LOCAL_REGISTRY ?= registry.goharbor.io
PLAYWRIGHT_LOCAL_REGISTRY_NAMESPACE ?= harbor-ci
...
playwright-test:
	...
	@set -e; \
		...
		$(DOCKERCMD) run --rm --network $(PLAYWRIGHT_DOCKER_NETWORK) \
			...
			-e LOCAL_REGISTRY=$(PLAYWRIGHT_LOCAL_REGISTRY) \
			-e LOCAL_REGISTRY_NAMESPACE=$(PLAYWRIGHT_LOCAL_REGISTRY_NAMESPACE) \
			-e DOCKER_HOST=$(PLAYWRIGHT_DOCKER_HOST) \
			-e PLAYWRIGHT_REPORTER=$(PLAYWRIGHT_REPORTER) \
			-e SCANNER_ENDPOINT=$(PLAYWRIGHT_SCANNER_ENDPOINT) \
			-e SCANNER_PORT=$(PLAYWRIGHT_SCANNER_PORT) \
