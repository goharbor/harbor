...
*/
UPDATE execution
SET revision=0, status=(SELECT status FROM task WHERE execution.id=task.execution_id)
WHERE execution.vendor_type='SCHEDULER' AND execution.revision IS NULL;