...
-- Seed permissions for built-in roles into permission_policy + role_permission.
-- These were previously hardcoded in Go (rolePoliciesMap); storing them in DB lets
-- the anti-escalation check compare any role's permissions against the caller's.
DO $$
DECLARE
