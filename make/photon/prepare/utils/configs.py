...
from models import InternalTLS, Metric, IPv6
...
def parse_yaml_config(config_file_path, with_notary, with_trivy, with_chartmuseum):
    ...
    # IPv6
    ipv6_config = configs.get('ipv6')
    if ipv6_config:
        config_dict['ipv6'] = IPv6(ipv6_config['enabled'], ipv6_config['subnet'], ipv6_config['gateway'])
    else:
        config_dict['ipv6'] = IPv6()
